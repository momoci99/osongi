...
import type { PriceDatum } from "./chartHelpers";

type UseDrawGradeBarPriceParams = {
  data: PriceDatum[];
  height: number;
  yMaxOverride?: number;
  labelYOffset: number;
...
/**
 * 등급별 평균 단가 막대 차트를 D3로 그리는 훅입니다.
 */
export const useDrawGradeBarPrice = ({
  data,
  height,
  yMaxOverride,
  labelYOffset,
  theme,
}: UseDrawGradeBarPriceParams) => {
  ...
  const draw = useCallback(() => {
    ...
    const barHover =
      theme.palette.mode === "dark"
        ? theme.palette.chart.price.main
        : theme.palette.chart.price.dark;
...
export default useDrawGradeBarPrice;
